...
export function TripNavigationGuard() {
  const { hasUnsavedChanges, isSaving } = useTripEditor()
  ...
  useEffect(() => {
    ...
    const handlePopState = (e: PopStateEvent) => {
      if (!hasUnsavedChanges || isNavigating.current || isSaving) return
  ...
  }, [hasUnsavedChanges, pathname, isSaving])
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (hasUnsavedChanges && !isNavigating.current && !isSaving) {
        e.preventDefault()
        e.stopPropagation()
        setShowDialog(true)
        setPendingUrl(href)
