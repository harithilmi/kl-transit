...
export function RouteStopList({ services }: RouteStopListProps) {
  ...
  const direction1Services = sortedServices.filter(service => service.direction === 1)
  const direction2Services = sortedServices.filter(service => service.direction === 2)
        ...
        <div className="flex w-full justify-between">
          ...
          <div className="flex-1 flex flex-col h-full justify-between">
            {direction1Services.map(
              (service) =>
                ((!isSharedFirstStop || service.stop_id !== direction1First?.stop_id) &&
                (!isSharedLastStop || service.stop_id !== direction1Last?.stop_id)) && (
                  <div
                    className="hover:bg-white/10 px-6 rounded-lg transition-colors duration-300 pr-10 py-4 relative after:absolute after:top-0 after:right-0 after:w-1 after:h-full after:bg-indigo-600 after:translate-x-[-1.25rem] grow min-h-0"
                    key={service.stop_id}
                  >
          ...
          <div className="flex-1 flex flex-col h-full justify-between">
            {[...direction2Services].reverse().map(
              (service) =>
                ((!isSharedFirstStop || service.stop_id !== direction2Last?.stop_id) &&
                (!isSharedLastStop || service.stop_id !== direction2First?.stop_id)) && (
                  <div
                    className="hover:bg-white/10 px-6 rounded-lg transition-colors duration-300 pl-10 py-4 relative after:absolute after:top-0 after:left-0 after:w-1 after:h-full after:bg-indigo-600 after:translate-x-[1.25rem] grow"
                    key={service.stop_id}
                  >
