...
export function RecentRoutes({ routes }: { routes: Route[] }) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setRecentRoutes(parsed.sort((a, b) => b.timestamp - a.timestamp).slice(0, 3))
    }
  }, [])
  ...
  return (
    <Card className="w-full max-w-xl p-4">
      <div className="flex flex-col gap-4">
        <h2 className="text-xl font-semibold">{t('HomePage.recentRoutes.title')}</h2>
